...
def _system_for_plan(plan: Plan) -> str:
    scope_label = plan.scope.label if plan.scope else "the loaded competition/season"
    return f"""You are a football analyst writing the answer for PitchMind. You are given the
user's question and the EXACT result rows from a verified database query ({scope_label}).
...
def synthesize(question: str, plan: Plan, result: ExecResult) -> str:
    ...
    payload = {
        "question": question,
        "metric": plan.metric,
        "scope": plan.scope.label if plan.scope else plan.time_scope,
    ...
    answer = llm.complete(_system_for_plan(plan), user, max_tokens=700)
